...
MARS_MIPS = "./external/Mars4_5.jar"
...
class TestCodeGen():
    ...
    def check(soldir,asttree,num):
        ...
        try:
            ...
            cmd = "java -jar "+ MARS_MIPS + " " + path + "/extra_ass.asm"
            ...
            f.close()
            with open('temp.txt', 'w') as temp, open(os.path.join(soldir, str(num) + ".txt"),"r") as rf:
                lines = rf.readlines()
                for i, line in enumerate(lines):
                    if not line.startswith('MARS 4.5') and not line.startswith('Warning') and not line.startswith('\n'):
                        if i == len(lines) - 1 and line.endswith('\n'):
                            line = line.rstrip('\n')
                        temp.write(line)
            os.replace('temp.txt', os.path.join(soldir, str(num) + ".txt"))
            temp.close()
            rf.close()
